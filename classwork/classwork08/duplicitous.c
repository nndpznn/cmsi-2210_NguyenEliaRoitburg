...
int main( int argc, char * argv[] ) {
  ...
  if (!out) {
    printf("File output.txt could not be opened or does not exist.\n");
  }
   ...
   */
   for (int i = 0; i < copyCount; i++) {
    for (int k = 0; k < blocksize; k++) {
        fgets(lineBuffer, MAX_CHAR_COUNT, in);
        fputs(lineBuffer, out);
    }
    fputs(newline, out);                        // Prints this weird character, but does skip a line. 
    rewind(in);
    fgets(lineBuffer, MAX_CHAR_COUNT, in);
   }
